   ...
   const handleSubmit = async (e: { preventDefault: () => void; }) => {
        ...
        try {
            if ( senha !== repetirSenha ) {
                Swal.fire({
                    title: 'Erro!',
                    text: 'A senha e Repetir Senha não estão iguais.',
                    icon: 'error',
                    confirmButtonText: 'ok'
                  })
            }
            else if (nome.length > 0 && email.length > 0 && senha.length > 0 && repetirSenha.length > 0){
                await User.createUser(email, nome, senha, 'user').then((res) => {
    ...
    return (
        <div className="grid grid-cols-2 h-screen bg-[#ed4742] m-auto justify-center items-center">
            ...
            items-center shadow-2xl relative  justify-between max-sm:col-span-2 max-sm:w-[80%] ">
                ...
                <h2 className="text-black text-center text-xl font-light italic mt-1  ml-1 mr-1">Cadastre-se e faça seu pedido em instantes!</h2>
                <form className="mt-5" onSubmit={handleSubmit}>
                <InputForm placeholder="Nome" input={nome} setInput={setNome}/>
                <InputForm placeholder="Email" input={email} setInput={setEmail}/>
                <InputForm placeholder="Senha" input={senha} setInput={setSenha}/>
                <InputForm placeholder="Repetir Senha" input={repetirSenha} setInput={setRepetirSenha}/>
                ...
                <p className="text-[#423f32] text-center mt-10">Já tem uma conta?</p>
                <button className="text-white bg-[#ed4742] w-[80%] h-10 rounded-lg active:scale-90 active:bg-red-700 transition-transform shadow-2xl
                    mt-5 mr-auto ml-auto flex justify-center items-center
                    " onClick={() => router.push('/login')}>Entrar</button>
            </div>
